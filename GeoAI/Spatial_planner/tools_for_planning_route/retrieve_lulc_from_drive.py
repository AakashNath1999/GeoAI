...
def retrieve_lulc_from_drive_main():
    """
    Retrieves the LULC file for the current task_id from Google Drive
...
# Authenticate Google Drive
    ...
    # Step 3: Search and download LULC
    drive = GoogleDrive(gauth)
    file_list = drive.ListFile({
        'q': f"title='{lulc_name}' and trashed=false",
        'corpora': 'allDrives',
    ...
    if file_list:
        file = file_list[0]
        output_path = os.path.join(download_dir, lulc_name)
        file.GetContentFile(output_path)
        print(f" Downloaded {lulc_name} to {output_path}")
    else:
        print(f" {lulc_name} not found in Drive.")
