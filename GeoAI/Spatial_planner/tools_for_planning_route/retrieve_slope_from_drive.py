...
CLIENT_SECRET_PATH = SCRIPT_DIR / "client_secrets.json"
...
def retrieve_slope_from_drive_main():
    """
    Retrieves the slope raster for the current task_id from Google Drive
    ...
    slope_name = f"slope_{task_id}.tif"
   ...
   # Authenticate Google Drive
    ...
    # Step 3: Search and download slope file
    drive = GoogleDrive(gauth)
    file_list = drive.ListFile({
        'q': f"title='{slope_name}' and trashed=false",
        'corpora': 'allDrives',
    ...
    if file_list:
        file = file_list[0]
        output_path = os.path.join(download_dir, slope_name)
        file.GetContentFile(output_path)
        print(f" Downloaded {slope_name} to {output_path}")
    else:
        print(f" {slope_name} not found in Drive.")
