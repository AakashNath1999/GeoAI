...
def download_osm_features(polygon, tags={"highway": True}):
...
def rasterize_vector(gdf, bounds, resolution_m=10, out_path="output.tif"):
    ...
    accounting for meters/degree at bbox center latitude.
    """
    minx, miny, maxx, maxy = bounds
    lat_c = (miny + maxy) / 2.0
...
def download_highways_main(resolution_m=10):
    ...
    # 3) Download OSM highways within AOI
    gdf = download_osm_features(bbox_poly, tags={"highway": True})
    if gdf.empty:
        print(f"⚠ No highway data found for AOI covering: {start_place} → {end_place}")
        return

    # 4) Output path from utils
    output_folder, _ = get_project_paths()
    out_path = os.path.join(output_folder, f"osm_roads_{task_id}.tif")

    # 5) Rasterize
    rasterize_vector(gdf, bbox_coords, resolution_m=resolution_m, out_path=out_path)
    print(f" Road raster saved: {out_path} | AOI: {start_place} → {end_place}")
